...
from numpy import append, array, bool_, exp, tanh, ndarray, ones, nan
...
# FUNCTIONS
###############################################################################
only_positive = lambda y: y * (y >= 0) + 0 * (y < 0)
...
class Individual(BaseClass):
    ...
    def append_history(self, extra=None):
        """
    ...
    def history(self, id=None, keys=None):
        ...
        #
        if id == None:
            return dict((key, value) for (key, value) in H.iteritems() if key in keys)
        else:
            return dict((key, value[:, 0])  for (key, value) in H.iteritems() if key in keys)
    ...
    #@property
    #def history(self):
    #    """Time-series of the history of the individuum."""
    #    return self._get_attribute('history')
    #@history.setter
    #def history(self, a):
    #    self._set_attribute('history', a)
